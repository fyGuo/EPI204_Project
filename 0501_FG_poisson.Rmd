...
dta$log_year <- log(dta$fu_time)
fit1_q <- glm(cancer_death ~ BOOZE_q, data = dta, 
              offset = log_year,
              family =poisson())
fit1_q %>% summary()
beta <- coef(fit1_q)
...
fit1_con <- glm(cancer_death ~ BOOZE, data = dta, 
              offset = log_year,
              family =poisson())
fit1_con %>% summary()
beta <- coef(fit1_con)
...
fit1_ord <- glm(cancer_death ~ BOOZE_ord, data = dta, 
              offset = log_year,
              family =poisson())
fit1_ord %>% summary()
...
## Adjust for age
```{r}
fit2_q <- glm(cancer_death ~ BOOZE_q + AGEYRS, data = dta, , 
              offset = log_year,
              family =poisson())
fit2_q %>% summary()
beta <- coef(fit2_q)
...
fit2_con <- glm(cancer_death ~ BOOZE + AGEYRS, data = dta, 
              offset = log_year,
              family =poisson())
fit2_con %>% summary()
beta <- coef(fit2_con)
...
fit2_ord <- glm(cancer_death ~ BOOZE_ord + AGEYRS, data = dta, 
              offset = log_year,
              family =poisson())
fit2_ord %>% summary()
